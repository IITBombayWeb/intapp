...
class ExcelImportExportController extends ControllerBase {
  ...
  public function excelContentExportSettings() {
    ...
    if (!empty($_GET['min']) && !empty($_GET['max'])) {
      $query->condition('n.created', [strtotime($_GET['min']), strtotime($_GET['max'])], 'BETWEEN');
    }
    ...
    $batch = [
      'title' => $this->t('Exporting Applications'),
      'operations' => [
        [
          'excel_import_export_batch_export',
          [
            $nids,
            $file_path,
            $zip_path,
