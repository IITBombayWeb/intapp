...
class TablefieldWidget extends WidgetBase {
  ...
  public function formElement(FieldItemListInterface $items, $delta, array $element, array &$form, FormStateInterface $form_state) {
   ...
   $data = $this->getFieldSettings(); // fetching data from storage setting form.
   $config = \Drupal::service('config.factory')->getEditable('tablefield.settings');
   $config->set('cols', $data['cols'])->save();  // variable set for column number.
    ...
    $element = array(
      ...
      '#addrow' => \Drupal::currentUser()->hasPermission('addrow tablefield'), // declare addrow permission.
      // assign all value from storage setting form to element.
      '#colnum' => $data['numbers'], 
      '#col_opt' => $data['col_opt'],
      '#col_date' => $data['column_date'],
    )+$element;
  ...
  function Tablefield_form_node_form_alter(&$form, FormStateInterface $form_state) {
  }
