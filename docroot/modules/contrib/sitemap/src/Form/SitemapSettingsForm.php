...
class SitemapSettingsForm extends ConfigFormBase {
  ...
  /**
   * An array of SitemapMap plugins.
   */
  protected $plugins = [];
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    foreach ($this->plugins as $id => $plugin) {
      ...
      $form['plugins']['enabled'][$id] = array(
        '#type' => 'checkbox',
        '#title' => $plugin->getLabel(),
        '#default_value' => $plugin->enabled,
        '#parents' => array('plugins', $id, 'enabled'),
      ...
      $form['plugins']['order'][$id]['filter'] = array(
        '#markup' => $plugin->getLabel(),
      );
      $form['plugins']['order'][$id]['weight'] = array(
        '#type' => 'weight',
        '#title' => $this->t('Weight for @title', array('@title' => $plugin->getLabel())),
        '#title_display' => 'invisible',
        '#delta' => 50,
        '#default_value' => $weight,
        '#parents' => array('plugins', $id, 'weight'),
        '#attributes' => array('class' => array('plugin-order-weight')),
      );

      // Retrieve the settings form of the SitemapMap plugins.
      $settings_form = array(
        '#parents' => array('plugins', $id, 'settings'),
        '#tree' => TRUE,
      );
      $settings_form = $plugin->settingsForm($settings_form, $form_state);
      if (!empty($settings_form)) {
        $form['plugins']['settings'][$id] = array(
          '#type' => 'details',
          '#title' => $plugin->getLabel(),
          '#open' => TRUE,
          '#weight' => $weight,
