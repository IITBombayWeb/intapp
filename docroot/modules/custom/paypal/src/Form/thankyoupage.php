	     ...
	     if ($tx_status == 'SUCCESS'){ */
		  if ($result['payment_status'] == 'pending' && $result['before_amount'] == $_GET['amt']){
			...
			$gender = $profile->get('field_gender')->getValue()[0]['value'];
		       // $nationality = $profile->get('field_nationality')->getValue()[0]['value'];
			...
			//$email_id = $profile->get('field_email_id')->getValue()[0]['value'];
			...
			<tr>
		     ...
		     $perform_pgm_apply_result .= "<tr>
		      ...
		      <td>".$specialisation."</td></tr></table>";
			       ...
			       //$profile->get('field_medium_of_instruction_')->getValue()[0]['value']
			    ...
			    if($send_mail_user == '1') {
			      ...
			      $result = $mailManager->mail($module, $key, $to, $langcode, $params, NULL, $send);
				   if ($result['result'] === true) {
				     //drupal_set_message(t('Application details sent to your registered mail.'));
			...
			//drupal_set_message("Thank you! Your transaction is successful and applied to " . count($applicationArray) . " programmes. Please note your transaction ID : ".$_GET['tx']." for future reference");		 
